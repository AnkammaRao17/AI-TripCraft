...
export function getDestinationImageUrl(name: string): string {
...
export function getNextDestinationImageUrl(currentUrl: string, name: string): string {
  if (!name) return getFallbackImage('Destination');
  const normalized = name.toLowerCase().trim().replace(/,\s*\w+$/, '');
  const list = DESTINATION_IMAGES[normalized] || [];

  const baseCurrent = currentUrl.split('?')[0];
  const idx = list.findIndex(item => item.split('?')[0] === baseCurrent);
