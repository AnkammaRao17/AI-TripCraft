        ...
        <mat-card-content>
          <form [formGroup]="resetForm" (ngSubmit)="onSubmit()" class="auth-form">
            ...
            <mat-form-field appearance="outline" class="full-width">
              ...
              <mat-error *ngIf="resetForm.get('password')?.hasError('minlength')">Must be at least 6 characters</mat-error>
            </mat-form-field>
            ...
            <mat-form-field appearance="outline" class="full-width">
              ...
              <mat-error *ngIf="resetForm.hasError('mismatch') && resetForm.get('confirmPassword')?.touched">
                Passwords do not match
              </mat-error>
            </mat-form-field>

            <!-- Submit Button -->
            <button mat-raised-button type="submit" class="full-width btn-primary submit-btn" [disabled]="resetForm.invalid || isLoading()">
  ...
  onSubmit(): void {
    ...
    this.authService.resetPassword(this.token(), this.resetForm.value.password).subscribe({
      next: () => {
        ...
        this.notification.success('Password updated successfully! Please login.');
        this.router.navigate(['/auth/login']);
      },
      error: (err) => {
        this.isLoading.set(false);
        const errMsg = err.error?.message || 'Token is invalid or has expired.';
